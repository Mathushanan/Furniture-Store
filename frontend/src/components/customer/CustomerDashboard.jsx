...
const CustomerDashboard = () => {
  ...
  // Populate from saved design
  useEffect(() => {
    if (designData) {
      setDesignName(designData.designName || "");
      setWallColor(designData.wallColor || "#ffffff");
      setWallHeight(designData.wallHeight || 3);
      setWallThickness(designData.wallThickness || 0.2);
      setRoomSize(designData.roomSize || 10);
      setSelectedWall(designData.wallTexture || "Sand Stone Wall");
      setSelectedFloor(designData.floorTexture || "Tile Floor");
      setViewMode(designData.viewMode || "3D");

      // Transform furnitures (if needed) before setting
      if (Array.isArray(designData.furnitures)) {
        const withIds = designData.furnitures.map((f, index) => ({
          id: `furniture-${index}`,
          ...f,
        }));
        setFurnitureList(withIds);
      }
    }
  }, [designData]);
  ...
  const handleSaveResult = (msg, type) => {
